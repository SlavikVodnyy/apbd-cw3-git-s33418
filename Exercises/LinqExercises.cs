...
public sealed class LinqExercises
{
    ...
    {
        return [UniversityData.Lecturers.All(l => l.Department != null) ? "Yes" : "No"];
    }
    ...
    public IEnumerable<string> Task16_HighestGradePerStudent()
    {
        return UniversityData.Students
            ...
            .Select(g => $"{g.Key} {g.Max(se => se.e.FinalGrade)}");
    }
    ...
    public IEnumerable<string> Challenge02_AprilCoursesWithoutFinalGrades()
    {
        return UniversityData.Courses
            ...
            .GroupJoin(UniversityData.Enrollments,
             c => c.Id,
             e => e.CourseId,
             (c, enrollments) => new { c, enrollments })
            .Where(ce => ce.enrollments.All(e => e.FinalGrade == null))
            .Select(ce => ce.c.Title);
    }
    ...
    public IEnumerable<string> Challenge03_LecturersAndAverageGradeAcrossTheirCourses()
    {
        return UniversityData.Lecturers
            ...
            .GroupJoin(UniversityData.Enrollments,
             lc => lc.courses.Select(c => c.Id).FirstOrDefault(),
             e => e.CourseId,
             (lc, enrollments) => new { lc.l, enrollments })
            .Where(le => le.enrollments.Any(e => e.FinalGrade != null))
            .Select(le => $"{le.l.FirstName} {le.l.LastName} {le.enrollments.Average(e => e.FinalGrade)}");
    }
